...
TCanvas* FitPlotter::PlotRegionSignificance(const string& can_name,
    ...
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(11111111);
    auto can = new TCanvas(Form("can_%s", can_name.c_str()),
                           Form("can_%s", can_name.c_str()),
                           1200, 700);
...
void FitPlotter::CombineBins(const string& can_name,
    ...
    if(cType == kVis){
        for(int s = 0; s < Nsig; s++){
            ...
            mergedBinList.SetIdentifier(CT.GetBareLabel());
...
std::string extractLeftString(std::string str, std::string c) {
    std::string result = "";
    size_t index = str.find(c);
    if (index != std::string::npos) {
        size_t leftIndex = str.rfind(' ', index);
        if (leftIndex != std::string::npos && leftIndex != index-1) {
            result = str.substr(leftIndex+1, index-leftIndex);
        }
    }
    return result;
}

SuperBinList FitPlotter::CombineBinsSigAvg(const VS& proc_bkg,
                                           const VS& proc_sig,
                                           const CategoryTree& CT,
                                           const double SF,
                                           const double sys,
                                           const bool concise,
                                           const CombineType cType){
    ...
    if(Nsig == 0 ||
       ...
       Nvis == 0 ||
       Ncat == 0)
        throw std::runtime_error("Input objects cannot be empty!");

    vector<int> Nbin;
    vector<string> jLabels;
    vector<FitBin> fitbins;
    map<string, FitBin> unique_fitbins;
    for(int v = 0; v < Nvis; v++) {
        CategoryList cat = CatList.Filter(*CatTrees[v]);
        string jID = extractLeftString(string(CatTrees[v]->GetPlainLabel(Depth)), "J ");
        const FitBin& fitbin = cat[0].GetFitBin();
        Nbin.push_back(fitbin.NBins());
        fitbins.push_back(fitbin);
        if(jID.empty())
            jID = "default";
        jLabels.push_back(jID);
        unique_fitbins.insert(std::make_pair(jID, fitbin));
    }
    ...
    //background
    vector<std::tuple<string, vector<double>>> label_bkg;
    vector<vector<double>> nBkgEvts(Nvis);
    for(int v = 0; v < Nvis; v++){
        CategoryList cat = CatList.Filter(*CatTrees[v]);
        vector<double> hbkgVec = GetAddedHistValues(cat, bkgs);
        nBkgEvts[v] = hbkgVec;
        label_bkg.emplace_back(pair<string, vector<double>>(jLabels[v], hbkgVec));
    }

    //signal
    vector<std::tuple<string, vector<double>>> label_sig;
    vector<vector<double>> nSigEvts(Nvis);
    XsecTool g_Xsec;
    double normXsec = 0.;
    ...
    for(int s = 0; s < Nsig; s++){
        ...
        for(int v = 0; v < Nvis; v++){
            ...
            if(nSigEvts[v].empty()){
                for(int i = 0; i < Nbin[v]; i++)
                    hsigVec[i] /= xsec;
                nSigEvts[v] = hsigVec;
            }
            else
                for(int b = 0; b < Nbin[v]; b++)
    ...
    for(int v = 0; v < Nvis; v++) {
        for (int b = 0; b < Nbin[v]; b++)
            nSigEvts[v][b] *= normXsec / SF;
        label_sig.emplace_back(pair<string, vector<double>>(jLabels[v], nSigEvts[v]));
    }
    SuperBinList mergedBinList;
    mergedBinList.SetIdentifier(CT.GetBareLabel());

    //SuperBinList CompleteBinList;
    //mergedBinList.SetIdentifier("Everything");

    //combine bins across mperp risr bins
    if(cType == kVis){
        for(int v = 0; v < Nvis; v++){
            int NM = 0;
            int index = 0;
            ...
            for(int r = 0; r < fitbins[v].NRBins(); r++){
                ...
                VS mlabel;
                rlabel += fitbins[v][r].GetRBinLabel();
                for(int m = 0; m < NM; m++){
                    ...
                    auto sb = new SuperBin(vidx, nSigEvts[v][index+m], nBkgEvts[v][index+m], ml, rl, vis);
                    sBinList += sb;
        ...
        mergedBinList.PrintSummaryVis(sys, concise);
    }

    //combine bins within mperp risr bin
    if(cType == kMR) {
        for (auto const &pair: unique_fitbins) {
            int index = 0;
            VS rlabel;
            int NR = pair.second.NRBins();
            for (int r = 0; r < NR; r++) {
                int NM = pair.second[r].NBins();
                VS mlabel;
                rlabel += pair.second[r].GetRBinLabel();
                VS rl((string(rlabel[r])));
                for (int m = 0; m < NM; m++) {
                    mlabel += pair.second[r].GetMBinLabels();
                    VS ml((string(mlabel[m])));
                    SuperBinList sBinList;
                    for (int v = 0; v < Nvis; v++) {
                        VI vidx((v));
                        VS vis((string(CatTrees[v]->GetPlainLabel(Depth))));
                        if (std::strcmp(pair.first.c_str(), std::get<0>(label_bkg[v]).c_str()) == 0) {
                            auto sb = new SuperBin(vidx, nSigEvts[v][index + m], nBkgEvts[v][index + m],
                                                   ml, rl, vis);
                            sBinList += sb;
                        }
                    }
        ...
        mergedBinList.PrintSummary(sys, concise);
    }
    return mergedBinList;
}

SuperBinList FitPlotter::GetAllBins(const VS& proc_bkg,
                                    const VS& proc_sig,
                                    const CategoryTree& CT,
                                    const double SF,
                                    const double sys,
    ...
    for(int v = 0; v < Nvis; v++) {
        ...
        for (int r = 0; r < fitbins[v].NRBins(); r++) {
            ...
            for (int m = 0; m < NM; m++) {
                ...
                VS ml((string(mlabel[m])));
                VS rl((string(rlabel[r])));
                VS vis(CT.GetBareLabel()+" "+(string(CatTrees[v]->GetPlainLabel(Depth))));
...
SuperBin* FitPlotter::GetSuperBin(const VS& proc_bkg,
                                 const VS& proc_sig,
                                 const CategoryTree& CT,
    ...
    VS categoryIDs, RISRIDs, MperpIDs;
    VI indices;
    vector<const CategoryTree*> superbin_cts;
    for(auto const &id : binIDs) {
        string id_name = std::get<0>(id);
        for (int i = 0; i < Nvis; i++) {
            string cat_id = CatTrees[i]->GetPlainLabel(Depth);
            if (strcmp(cat_id.c_str(), id_name.c_str()) == 0) {
                superbin_cts.push_back(CatTrees[i]);
                categoryIDs += std::get<0>(id);
                MperpIDs += std::get<1>(id);
                RISRIDs += std::get<2>(id);
                indices += i;
            }
        }
    }

    Nvis = int(superbin_cts.size());
    ...
    //signal
    XsecTool g_Xsec;
    //vector<double> signal_yield(Nsig, 0.);
    double signal_yield = 0;
    //for(int s = 0; s < Nsig; s++){

    auto mass_tuple = SigMass(proc_sig[0]);
    int parent_mass = std::get<0>(mass_tuple);
    std::string proc_name = std::get<2>(mass_tuple);
    ...
    for(int v = 0; v < Nvis; v++){
        ...
        /*for(int i = 0; i < int(hsigVec.size()); i++)
            cout << "value in bin " << i << ": " << hsigVec[i] << endl;*/
