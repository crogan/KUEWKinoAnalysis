...
TCanvas* combinedCanvas(const string& can, TH2D* hist, PlotType ptype);
...
void compareLimits(const string& json1, const string& json2, TString name, PlotType ptype = kT2tt, bool inclObs = false){
  gROOT->SetBatch(kTRUE);

  RestFrames::SetStyle();
  
  Limit* limit_def1 = new Limit(json1);
  Limit* limit_def2 = new Limit(json2);
  if(limit_def1 == NULL || limit_def2 == NULL) return;
  TLatex l;
  l.SetTextFont(42);
  l.SetNDC();
  ...
  //JSON 1
  TH2D*   hist_exp_dM1   = limit_def1->Get2DHist_dMvMP("h_exp_dM1", kExp);
  TGraph* gr_exp_dM1     = limit_def1->Get2DContour_dMvMP(kExp);
  TGraph* gr_exp_dM_up1  = limit_def1->Get2DContour_dMvMP(kExpUp);
  TGraph* gr_exp_dM_dn1  = limit_def1->Get2DContour_dMvMP(kExpDn);
  TGraph* gr_exp_dM_obs1 = limit_def1->Get2DContour_dMvMP(kObs);

  //JSON 2
  TH2D*   hist_exp_dM2   = limit_def2->Get2DHist_dMvMP("h_exp_dM2", kExp);
  TGraph* gr_exp_dM2     = limit_def2->Get2DContour_dMvMP(kExp);
  TGraph* gr_exp_dM_up2  = limit_def2->Get2DContour_dMvMP(kExpUp);
  TGraph* gr_exp_dM_dn2  = limit_def2->Get2DContour_dMvMP(kExpDn);
  TGraph* gr_exp_dM_obs2 = limit_def2->Get2DContour_dMvMP(kObs);
  
  TH2D* h_combined = (TH2D*)hist_exp_dM1->Clone();
  h_combined->Divide(hist_exp_dM2);
  if(ptype == kTChiWZ){
    h_combined->GetXaxis()->SetRangeUser(100, 600);
    h_combined->GetYaxis()->SetRangeUser(3, 50);
    //h_combined->GetZaxis()->SetRangeUser(0.5, 1);
  }

  if(ptype == kT2tt){
    h_combined->GetXaxis()->SetRangeUser(250, 1050);
    h_combined->GetYaxis()->SetRangeUser(10, 80);
  }

  TCanvas* can_dM = combinedCanvas("can_dM", h_combined, ptype);
  can_dM->cd();
  
  gr_exp_dM1->SetLineColor(7043);
  gr_exp_dM1->SetLineWidth(5);
  gr_exp_dM1->SetLineStyle(1);
  gr_exp_dM1->Draw("same C");

  gr_exp_dM_up1->SetMarkerColor(kWhite);
  gr_exp_dM_up1->SetLineColor(7043);
  gr_exp_dM_up1->SetLineWidth(4);
  gr_exp_dM_up1->SetLineStyle(7);
  gr_exp_dM_up1->Draw("same C");
  gr_exp_dM_dn1->SetMarkerColor(kWhite);
  gr_exp_dM_dn1->SetLineColor(7043);
  gr_exp_dM_dn1->SetLineWidth(4);
  gr_exp_dM_dn1->SetLineStyle(7);
  gr_exp_dM_dn1->Draw("same C");
  gr_exp_dM_obs1->SetMarkerColor(kWhite);
  gr_exp_dM_obs1->SetLineColor(kBlack);
  gr_exp_dM_obs1->SetLineWidth(4);
  gr_exp_dM_obs1->SetLineStyle(1);
  if(inclObs) gr_exp_dM_obs1->Draw("same C");

  gr_exp_dM2->SetLineColor(kBlue+2);
  gr_exp_dM2->SetLineWidth(5);
  gr_exp_dM2->SetLineStyle(1);
  gr_exp_dM2->Draw("same C");

  gr_exp_dM_up2->SetMarkerColor(kWhite);
  gr_exp_dM_up2->SetLineColor(kBlue+2);
  gr_exp_dM_up2->SetLineWidth(4);
  gr_exp_dM_up2->SetLineStyle(7);
  gr_exp_dM_up2->Draw("same C");
  gr_exp_dM_dn2->SetMarkerColor(kWhite);
  gr_exp_dM_dn2->SetLineColor(kBlue+2);
  gr_exp_dM_dn2->SetLineWidth(4);
  gr_exp_dM_dn2->SetLineStyle(7);
  gr_exp_dM_dn2->Draw("same C");
  gr_exp_dM_obs2->SetMarkerColor(kWhite);
  gr_exp_dM_obs2->SetLineColor(kBlack);
  gr_exp_dM_obs2->SetLineWidth(4);
  gr_exp_dM_obs2->SetLineStyle(1);
  if(inclObs) gr_exp_dM_obs2->Draw("same C");
  
  l.SetTextAlign(12);
  l.SetTextSize(0.035);
  l.SetTextFont(42);
  l.DrawLatex(0.23, 0.83,"New");
...
TCanvas* combinedCanvas(const string& name, TH2D* hist, PlotType ptype){
   TCanvas* can = (TCanvas*) new TCanvas(name.c_str(),name.c_str(),700.,600);
  ...
  hist->GetZaxis()->SetLabelFont(42);
  hist->GetZaxis()->SetLabelSize(0.045);
  hist->GetZaxis()->SetTitle("95% C.L. cross-section ratio");
  hist->Draw("COLZ");
