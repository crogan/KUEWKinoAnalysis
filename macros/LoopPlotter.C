#include "TSystem.h"

#include "../include/PlotReader.hh"

void LoopPlotter(const string& inputfile = "BPI_2017.root"){
  string year = "2017";

  
  PlotReader* plotter = new PlotReader(inputfile);
  plotter->SetPlotTitle("CR "+year+" Data + Simulation");
 
  VS proc;
  proc.a("ttbar").a("ST").a("DB").a("ZDY").a("TB").a("QCD").a("Wjets").a("Data");
  VS proc_obj;
  proc_obj.a("Bkg").a("Data");
  
  VS var_kin;
  var_kin.a("MET").a("RISR").a("Mperp").a("PTISR").a("Gperp");
  VS var_cnt;
  var_cnt.a("NSjet").a("Nbjet").a("NSbjet").a("NSV").a("Nel").a("Nmu");
  VS var_obj;
  var_obj.a("Eta").a("Phi").a("Pt");
  VS obj;
  obj.a("Jet").a("Bjet").a("El").a("Mu").a("SV");

  VS reg_kin;
  reg_kin.a("0L").a("0L_0J").a("0L_1J").a("0L_2J").a("0L_3J").a("0L_4J").a("0L_5J");
  //reg_kin.a("1L").a("1L_0J").a("1L_1J").a("1L_2J").a("1L_3J").a("1L_4J").a("1L_5J");

  for(auto r : reg_kin){
    for(auto v : var_kin){
      TCanvas* can;
      string scan = v+"_"+r+"_"+"year";
      if(v == "Mperp")
	can = (TCanvas*) plotter->Plot1Dstack(scan,v, proc, r, false, 4, 0., 650.);
      else if(v == "RISR")
	can = (TCanvas*) plotter->Plot1Dstack(scan,v, proc, r, false, 8);
      else
	can = (TCanvas*) plotter->Plot1Dstack(scan,v, proc, r, false, 4);
    }
  }


}
