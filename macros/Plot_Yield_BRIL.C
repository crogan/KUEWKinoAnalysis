...
void Plot_Yield_BRIL(){
  ...
  BRILTool bril;
  bril.BuildMap("json/BRIL/brilcalc_2017.txt");
  ...
  for(int s = 0; s < Nsample; s++){
	...
	double weight = (base->weight != 0.) ? base->weight : 1.;
        ...
        double IntegratedLumi = bril.GetIntegratedLumi(fill);
if(IntegratedLumi == 0) cout << "Integrated Lumi is zero! " << endl << "There may be a bug! " << endl;
...
//if(e > 5) break;	
  ...
  can->SaveAs("Plot_Yield_BRIL.pdf");
